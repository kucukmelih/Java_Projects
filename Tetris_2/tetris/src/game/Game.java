...
//*****************************************************************************
	...
	//*************************************************************
	private static final int NEXT_PIECE_Y = 16;
    //*************************************************************
	public static final int STARTING_PIECE_X = 4;
	public static final int STARTING_PIECE_Y = 19;
	...
	private void tick() {
		...
		if (this.field.isPieceFallen(currentPiece)) {
			...
			if(field.doesPieceTouchesTop(currentPiece)){
	...
	private void render() {
		...
		if (!this.isPaused()) {
			...
			if (!this.isRunning()) {
				// Set font and color for "Game Over" message
				this.graphics.setColor(Color.BLACK);
				this.graphics.setFont(new Font("Game Over", Font.BOLD, 75));

				// Calculate the center of the screen
				int centerX = this.display.getWidth() / 2;
				int centerY = this.display.getHeight() / 2;

				// Draw "Game Over" message at the center of the screen
				this.graphics.drawString("------------", centerX - 210, centerY - 65);
				this.graphics.drawString("------------", centerX - 210, centerY - 75);
				this.graphics.drawString("Game", centerX - 165, centerY - 15);
				this.graphics.drawString("Over!", centerX - 160, centerY + 65);
				this.graphics.drawString("------------", centerX - 210, centerY + 105);
				this.graphics.drawString("------------", centerX - 210, centerY + 115);
